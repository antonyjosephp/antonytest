...
//import { getBaggage } from "@opentelemetry/api/build/src/baggage/context-helpers";
import { Logging } from '@google-cloud/logging';
import { LogLevel } from './loglevel';
import { RequiredParamsValidatorError } from './RequiredParamsValidatorError';
/**
...
export class Logger {
    ...
    static writeLog(message: string) {
        try {
            //const x = api.propagation.getActiveBaggage()?.getEntry('X-Cloud-Trace-Context')?.value;
            ...
            if (isEqual) {
                ...
                throw new RequiredParamsValidatorError({
                    name: 'REQUIRED_PARAMETERS_IN_LOG_MISSING',
                    message: 'Following required parameters are missing from writelog method : ' + difference,
                    cause: Error,
                });
                //throw new Error('Following required parameters are missing from writelog method : ' + difference);
            }
        } catch (error: unknown) {
            if (error instanceof RequiredParamsValidatorError) {
